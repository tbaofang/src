...
void LGlidar::doProcessSimple(
  ...
  for (int i = 0; i < nRanges; i++) {
    ...
    //printf("outObservation.scan[i] is %f\n", outObservation.scan[i]);
...
bool LGlidar::receiveResponse(mrpt::obs::CObservation2DRangeScan &outObservation) {
  ...
  try {
    ...
    char buffer[43];
    int line_size = 0;
    while ((line_size = readline(buffer, 42)) > 0) {
...
int LGlidar::readline(char* buf, const size_t count)
{
    ...
    bool is_fill_FirstAndSecond = false;
    ...
    while (filled < count) {
        ...
        recv_ch = m_rx_buffer.pop();
         ...
         //cout<<"[readline]: ............................begin.......... ";
        //printf("%04x\n", recv_ch);
        //find the first of 0xFA
        if (!is_find_first)
        {
            if (!isFirstSync(recv_ch))
            {
                is_find_second = false;
                continue; 
            }
            
            is_find_first = true; //ok,find 0xFA, set the tag true,and don't into here again
            is_find_second = false;   
            continue;
        }
        //cout<<"[readline]: find it 1.....recv_ch is ";
        //printf("%04x\n", recv_ch);
        //find the second angle tag in(0xA0 - 0xDB)
        if (is_find_first && !is_find_second)
        {
            if (!isSecondAngle(recv_ch))
            {
                is_find_first = false;  //
                is_find_second = false;
                continue;
        ...
        //cout<<"[readline]: find it 2.....recv_ch is ";
        //printf("%04x\n", recv_ch);
        if (!is_fill_FirstAndSecond)
        {
            buf[filled++] = 0xFA;
            is_fill_FirstAndSecond = true;
        }

        //cout<<"[readline]: find it 2.....filled it ok...";
        //printf(",recv_ch is %02x\n", recv_ch);
...
bool LGlidar::addLengthData(std::vector<long>& data, char *line, mrpt::obs::CObservation2DRangeScan &outObservation)
  {
    ...
    int angle_intex = int((line[1]&0x00ff));

   

    //caculate the numbers of invalid distance,from the last distance to the first distance now
    
    //因为建图反转，偏移180度，
    //int n_diff = 6*(angle_intex-160) - data_size;
    int n_diff = 6*(angle_intex-160) - data_size -180;
    if(n_diff < 0) n_diff += 180;
    ...
    for (int i = 0; i < 6; i++)
    {
      ...
      low_intensity = line[i*6 + 4];
      high_intensity = line[i*6 + 5];

      lowNum = line[i*6 + 6];
      highNum = line[i*6 + 7];
      //printf("lowNum is %02x, highNum is %02x \n",lowNum, highNum);
      //decode intensity
      low_intensity &= 0x00ff;
      high_intensity <<= 8;
      high_intensity &= 0xffffff00;
      ...
      distance[i] = lowNum + highNum;
      intensity[i] = low_intensity + high_intensity;
    ...
    {
      if (n_diff > 180) return false; //if the invalid points is too much ,this 360 data mabe ignore
    ...
    for (int i = 0; i < 6; ++i)
    {
      ...
      //outObservation.setScanIntensity(6*(angle_intex-160) + i, intensity[i]);//here is some error?
