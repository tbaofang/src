...
private:
  ...
  /** Start the continuous scanning mode, using parameters stored in the object
   * (loaded from the .ini file). Maps to SCIP2.0 commands MD (no intensity) or
   * ME (intensity). After this command the device will start to send scans
   * until switchLaserOff() is called. \return false on any error
   */
